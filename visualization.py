...
def calculate_graph_layout(G:nx.Graph, min_h_distance: int = 1.5, min_v_distance: int = 2.0, iterations: int = 10) -> dict[int, tuple[int, int]]:
    ...
    final_coordinates = _finalize_coordinates(nodes_by_layer, positions, min_v_distance, min_h_distance, max_layer)
...
def _assign_layers(G: nx.Graph, reversed_adj) -> tuple[int, dict, dict]:
    """Calculates the layer (depth) for each node using DFS/Memoization."""
    memo = {}
    
    def get_depth(node, path_stack, reversed_adj):
        if node in memo:
            return memo[node]        
        if node in path_stack:
            return 0  # Prevent cycles    
        
        parents = reversed_adj[node]
        if not parents:
            return 0
        ...
        for parent in parents:
            parent_depth = get_depth(parent, path_stack, reversed_adj, memo)
...
def _optimize_node_order(nodes_by_layer, adjacency, reversed_adjacency, positions, min_h_distance, max_layer, iterations):
    ...
    for _ in range(iterations):
        ...
        for layer_index in range(1, max_layer + 1):
            _apply_barycenter_sort(layer_index, True, nodes_by_layer, reversed_adjacency, adjacency, positions, min_h_distance)
        # Up sweep
        for layer_index in range(max_layer - 1, -1, -1):
            _apply_barycenter_sort(layer_index, False, nodes_by_layer, reversed_adjacency, adjacency, positions, min_h_distance) 
...
def _finalize_coordinates(nodes_by_layer, positions, min_v_distance, min_h_distance, max_layer):
